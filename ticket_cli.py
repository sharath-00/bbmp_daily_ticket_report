...
def print_summary_report(analytics: dict, date_info: str):
    ...
    print(f"       Filter / Period: {date_info} (Lamp Tickets Only)")
    print(f"       (Note: SLC Panels & Automated 'Auto' tickets are excluded)")
    print("=" * 65)
    print(f"  --- LAMP TICKETS SUMMARY (RAISED FROM JULY 1ST TO DATE) ---")
    print(f"  Raised (From July 1) : {analytics.get('july1_total_tickets', 0)}")
    print(f"  Open (From July 1)   : {analytics.get('july1_open_tickets', 0)}")
    ...
    print(f"  Open / Active        : {analytics['open_tickets']}")
